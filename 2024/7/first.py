...
def workOnLine(target, accumulator, next:list, debugOutput):
    ...
    nextNum = next.pop(0)
    if workOnLine(target, accumulator + nextNum, list(next), debugOutput + ' + ' + str(nextNum)):
        return True
    return workOnLine(target, accumulator * nextNum, list(next), debugOutput + ' * ' + str(nextNum))
